...
public:
    ...
    void GrabSemtic(const sensor_msgs::ImageConstPtr& msgSemL,const sensor_msgs::ImageConstPtr& msgSemC);
...
//订阅语义标签和颜色并同步
    ...
    message_filters::Synchronizer<sync_pol2> sync2(sync_pol2(10), seml_sub,semc_sub);
    sync2.registerCallback(boost::bind(&ImageGrabber::GrabSemtic,&igb,_1,_2));
...
void ImageGrabber::GrabSemtic(const sensor_msgs::ImageConstPtr& msgSemL, const sensor_msgs::ImageConstPtr& msgSemC)
{
    ...
    mpSLAM->TrackSemtic(cv_ptrSemL->image, cv_ptrSemC->image);
}
//这是彩色图和深度图的回调

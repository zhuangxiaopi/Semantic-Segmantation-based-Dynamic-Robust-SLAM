...
void Tracking::GrabImageSem(const cv::Mat &imSemL, const cv::Mat &imSemC)
{
    cout << "--Tracking::GrabImageSem()语义图像赋值--" << endl;
    imSemL.copyTo(mImS_L);//将订阅的分割图像进行copy给mImS全局变量
    imSemC.copyTo(mImS_C);//将订阅的分割图像进行copy给mImS全局变量
}
...
void Tracking::CreateNewKeyFrame()
{
    ...
    {cout<<"mImS_L.size: "<<mImS_L.size<<endl;
        cout<<"mImS_C.size: "<<mImS_C.size<<endl;
        pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB,
                           this->mImRGB, this->mImDepth, this->mImS_L, this->mImS_C, this->mimMask);}
    else
        pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB);

...
void FrameDrawer::FillImage(cv::Mat &im, const cv::Mat &mask, const cv::Mat &Semlab, cv::Scalar scalar_mask, cv::Scalar scalar_label)
{
    unsigned char* color = (unsigned char*)im.data; //3 通道
    unsigned char* dyna = (unsigned char*)mask.data;//1 通道 动静 mask
    unsigned char* peop = (unsigned char*)Semlab.data;//1 通道 语义分割 Label
    ...
    for(int r=0; r<im.rows; r++ ) // y
    {
        for(int c=0; c<im.cols; c++) // x
        {
            ...
            if(peop[index]==31)//people label添加mask
            {
                color[index*3+0] = scalar_label.val[0];
                color[index*3+1] = scalar_label.val[1];
                color[index*3+2] = scalar_label.val[2];
            }
...
// 显示==========语义分割=====光流信息==动静信息===========
        ...
        if(!mask.empty()&&(s.val[0] > mask.rows * mask.cols * 0.65))
            FillImage(im, mask, label, cv::Scalar(255 , 0, 0), cv::Scalar(0 , 255, 255));
    }
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    pTracker->mimLabel.copyTo(mPeopLabel);// 语义分割标签People
